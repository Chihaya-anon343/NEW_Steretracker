...
{
    // NORM_HAMMING is required for AKAZE binary descriptors
    ...
    const std::vector<cv::KeyPoint>& kp_template)
{
    ...
    // Build reverse mapping: trainIdx(T) → queryIdx(L)
    // In T→L matches: queryIdx = template index, trainIdx = left index
    std::unordered_map<int, int> t2l_map;
    t2l_map.reserve(matches_t2l.size());
    for (const auto& m : matches_t2l) {
        t2l_map[m.queryIdx] = m.trainIdx; // template_idx → left_idx
    }

    // Cross-check: a L→T match is valid iff T→L maps back to the same left index
    ...
    // ---- Step 3: Homography RANSAC geometric consistency ----
