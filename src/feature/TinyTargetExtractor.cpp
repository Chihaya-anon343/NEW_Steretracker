...
// ============================================================================
// Public extract() — FeatureExtractor interface
// ============================================================================
...
PipelineResult TinyTargetExtractor::extract(const cv::Mat& left_gray,
    ...
    // ---- Step 1: Extract 4 corners from left ----
    ...
    // ---- Step 2: Extract 4 corners from right ----
    ...
    // ---- Step 3: Match left↔right by index (both TL→TR→BR→BL, 4 corners each) ----
    int n_stereo = (has_right && right_corners.size() == 4) ? 4 : 0;

    // ---- Step 4: Fill PipelineResult ----
    ...
    // 4b. Stereo data (disparity placeholder — computed in process() after offset)
    if (n_stereo > 0) {
    ...
    // 4c. Template matching data (1:1 correspondence by index)
    result.pts_left_match = left_corners;
    if (best_angle >= 0) {
        // Find matched template for visualization
        for (const auto& tmpl : templates_) {
            if (tmpl.angle == best_angle) {
                // Scale template corners to ROI size for pts_template_match
    ...
    // Synthetic DMatch for GPNP: 4 corners ↔ 4 template 3D points (1:1)
    for (int i = 0; i < 4; ++i)
        result.good_matches.emplace_back(i, i, 0.0f);

    // 4d. Generate 3D object points (rectangular target, z=0) — in MILLIMETERS
    {
        double half_mm = config_.square_size_m * 1000.0 / 2.0;  // m → mm
        template_data_.pts_3d = {
            {-half_mm, -half_mm, 0.0},  // TL
            { half_mm, -half_mm, 0.0},  // TR
            { half_mm,  half_mm, 0.0},  // BR
            {-half_mm,  half_mm, 0.0},  // BL
        };
    }
...
// ============================================================================
// Core: extract4Corners (migrated from legacy extractCorners + estimatePose)
// ============================================================================
...
Status TinyTargetExtractor::extract4Corners(const cv::Mat& roi_gray,
    ...
    // ---- 1. Otsu + template match → best angle ----
    cv::Mat roi_binary;
    ...
    // ---- 2. Super-resolution (×scale_factor) ----
    ...
    // ---- 3. Connected-component analysis + scoring ----
    ...
    // ---- 4. minAreaRect → 4 corners → orderPoints ----
...
// ============================================================================
// Template Matching (IoU-based, same as legacy)
// ============================================================================
...
TinyTargetExtractor::matchTemplate(const cv::Mat& roi_binary) {
    ...
    // Crop bounding square around largest component
    ...
    // Compute overlap with all templates, track best
...
// ============================================================================
// Component Selection (scoring system, same as legacy)
// ============================================================================
...
// ============================================================================
// Sub-pixel Corner Refinement (same as legacy)
// ============================================================================
