 ...
 * @brief ONNX Runtime YOLO object detector (header-only).
 *
 * Verified inference pipeline: letterbox → BGR→RGB → normalize → ONNX → decode.
 * Uses Detection / Status / DeviceType / YoloConfig types from common/Types.hpp.
 *
 * Constructor performs all ONNX initialization (throws on failure).
 */
...
// ============================================================================
// YOLO Object Detector (header-only)
// ============================================================================
...
public:
    ...
    /// Run detection on an image. On success, fills `detections`.
...
{
    // ---- Early validation: check file existence before creating ONNX session ----
    {
        ...
        // Quick sanity check by reading first 4 bytes (ONNX protobuf starts with 0x08)
        f.seekg(0);
        char header[4] = {};
        f.read(header, 4);
...
// --- Resolve input shape from ONNX model ---
...
// --- Preprocess: letterbox → BGR→RGB → normalize ---
