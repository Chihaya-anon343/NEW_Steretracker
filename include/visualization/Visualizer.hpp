...
private:
    ...
    /// Panel 1: Left+right stitched with matched-point connection lines
    ...
    /// Panel 2: Left image with projected right points
    void drawStereoProjection(const cv::Mat& left_color,
                              const std::vector<cv::Point2f>& pts_left_used,
                              const std::vector<cv::Point2f>& pts_right_proj,
                              const std::string& prefix);

    /// Panel 3: Left+template stitched with match lines
    ...
    /// Panel 4: Pose axes overlay on left image
