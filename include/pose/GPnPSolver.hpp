...
public:
    ...
    // Movable (stored by value, safe to move)
...
private:
    StereoCameraParams params_;  ///< Stored by value (enables move semantics)
    int gpnp_min_pts_;
    GPNPMonitor monitor_;

    /// Print monitor to stdout on failure (matches Python _print_gpnp_monitor).
